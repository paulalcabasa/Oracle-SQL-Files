...
--        rcta.creation_date transaction_date,
        ...
        ac.customer_name,        
        gjh.je_source,
        nvl(xal.description,gjl.description) description,
...
--           AND rcta.trx_number = '40200000873'    
          AND glcc.segment6 IN ('63000')
          AND gjh.je_source = 'Receivables'
          AND TRUNC(xah.accounting_date) <= '30-SEP-2017';
          


-- invoices correct 2 (used query as of july 10, 2017)
SELECT
        sum(gjl.accounted_dr) GL_ACCOUNTED_DR,
        sum(gjl.accounted_cr) GL_ACCOUNTED_CR,
        sum(gjl.accounted_dr) - sum(gjl.accounted_cr) gl_accounted_amount
FROM   
        xla.xla_ae_headers xah 
        INNER JOIN xla.xla_ae_lines xal
            ON  xah.ae_header_id = xal.ae_header_id 

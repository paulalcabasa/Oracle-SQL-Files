...
from (SELECT to_char(rcta.trx_date) trx_date,
...
GROUP BY trx_date,
           ...
           select to_char(rcta.trx_date),
                        regexp_replace(to_char(rcta.trx_date, 'Month'),'[[:space:]]+') ,
                        to_char(rcta.trx_date, 'YYYY')
           from ra_customer_trx_all rcta

             where cust_trx_type_id = 1080 
            AND regexp_replace(to_char(rcta.trx_date, 'Month'),'[[:space:]]+') = nvl(:p_month, regexp_replace(to_char(rcta.trx_date, 'Month'),'[[:space:]]+'))
            AND to_char(rcta.trx_date, 'YYYY') = nvl(:p_year,to_char(rcta.trx_date, 'YYYY'));
      
select *
from ra_customer_trx_all
where trx_number like '401%';
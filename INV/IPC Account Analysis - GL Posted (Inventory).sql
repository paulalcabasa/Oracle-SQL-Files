select regexp_substr('Transaction ID: 4952019 Item: 8942439281 Qty: 2 UOM: PCS Type: Return material authorization Org: DBS' ,'[^ Type: ]+',1,12)from dual;

SELECT   
--            REGEXP_SUBSTR (gjl.description, '[^ ]+', 1,3)    AS transaction_id,
--            REGEXP_SUBSTR (gjl.description, '[^ ]+', 1,5)    AS production_model,
--            REGEXP_SUBSTR (gjl.description, '[^ ]+', 1,7)    AS quantity,
--            REGEXP_SUBSTR (gjl.description, '[^ ]+', 1,9)    AS uom,
            REGEXP_SUBSTR (gjl.description, '[^Type:]+', 1,7) || REGEXP_SUBSTR (gjl.description, '[^: ]+', 1,8) || REGEXP_SUBSTR (gjl.description, '[^Type:]+', 1,9)  AS type,
          --  REGEXP_SUBSTR (gjl.description, '[^ ]+', 1,14)    AS org,
             gjl.description,
...
WHERE 1 = 1
        ...
        and gjh.doc_sequence_value = '81001127'
        AND REGEXP_SUBSTR (gjl.description, '[^ ]+', 1,3) = '4952019'
